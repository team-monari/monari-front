...
export default function EditStudy() {
    ...
    const checkAuth = async () => {
      if (!accessToken) {
        const result = await Swal.fire({
          title: '로그인 필요',
          text: '스터디 수정은 회원 기능입니다. 로그인하시겠습니까?',
          icon: 'question',
  ...
  useEffect(() => {
    const fetchStudyDetail = async () => {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/studies/${id}`, {
          headers: {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="container mx-auto px-6 py-12 max-w-[1280px]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">스터디 수정은 회원만 이용할 수 있는 기능입니다.</p>
            <button
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Swal.fire({
        title: '오류 발생',
        text: '스터디 수정 중 오류가 발생했습니다. 다시 시도해주세요.',
        icon: 'error',
        confirmButtonColor: '#1B9AF5',
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>스터디 수정 - 모나리</title>
        <meta name="description" content="모나리 스터디 수정 페이지" />
      </Head>
